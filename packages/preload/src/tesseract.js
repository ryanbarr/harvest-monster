...
import craft_keywords from "../assets/dictionaries/english/craft_keywords";
import tft_crafts from "../assets/dictionaries/english/tft_crafts";
...
export const tesseract = (blob) => {
    return new Promise(async (resolve) => {
        ...
        for (let i in cleanLines) {
            ...
            const lineTwo = cleanLines.length > index + 1 ? cleanLines[index + 1] : "";
            const lineThree =
                cleanLines.length > index + 2 ? cleanLines[index + 2] : "";
            const lineFour = cleanLines.length > index + 3 ? cleanLines[index + 3] : "";
            ...
            if (craft_keywords.hasOwnProperty(craftType)) {
                ...
                } else if (tft_crafts.hasOwnProperty(lineTwoCraft) && lineTwoLevel < 0) {
                    crafts.push({
                        id: Symbol("harvest_craft"),
                        key: lineTwoCraft,
